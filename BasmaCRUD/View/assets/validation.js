...
// Validation for Événement forms (title, description, date, location, capacity)
...
// Validation for Réservation forms (name, email, seats)
function validateReservationForm(form) {
    ...
    var seats = (form.seats && form.seats.value) ? form.seats.value.trim() : '';
    var eventId = (form.event_id && form.event_id.value) ? form.event_id.value.trim() : '';

    // clear previous invalid states
    try { _clearInvalid(form.name); _clearInvalid(form.email); _clearInvalid(form.seats); _clearInvalid(form.event_id); } catch(e){}

    var errors = [];

    // Name: 2-255 chars
    if (name.length < 2 || name.length > 255) {
        errors.push('Nom: entre 2 et 255 caractères.');
        if (form.name) _markInvalid(form.name);
    }
    ...
    if (email.length > 0) {
        ...
        if (!emailPattern.test(email)) {
            ...
            if (form.email) _markInvalid(form.email);
        }
    }

    // Seats: positive integer
    if (seats.length > 0 && !/^[0-9]+$/.test(seats)) {
        errors.push('Places: doit être un entier positif.');
        if (form.seats) _markInvalid(form.seats);
    }

    // Event ID: required
    if (!eventId) {
        errors.push('Événement: doit être sélectionné.');
        if (form.event_id) _markInvalid(form.event_id);
    }
...
// Backward compatibility: default to event validation
function validateForm(form) {
    // Detect form type by checking for specific fields
    if (form.name && form.seats && !form.title) {
        // Reservation form
