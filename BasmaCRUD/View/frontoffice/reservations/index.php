...
<body>
    ...
    <section class="section-with-form">
        ...
        <div class="filters">
            <button class="btn btn-add" onclick="document.getElementById('formModal').style.display='block';">➕ Nouvelle
                réservation</button>
        </div>
    </section>
    ...
    <div id="formModal" class="modal">
        <div class="modal-content">
            ...
            <form method="post" action="index.php?entity=reservation&action=store" onsubmit="return validateForm(this);"
                novalidate autocomplete="off" class="premium-form">

                <div class="form-group">
                    <label for="reservation_event_id">Événement</label>
                    <select name="event_id" id="reservation_event_id" class="form-control" autocomplete="off">
                        <?php if (!empty($events)): ?>
                            <?php foreach ($events as $ev): ?>
                                <option value="<?php echo $ev['id']; ?>" <?php echo isset($selectedEventId) && $selectedEventId == $ev['id'] ? 'selected' : ''; ?>>
                                    <?php echo htmlspecialchars($ev['title']); ?></option>
                            <?php endforeach; ?>
                        <?php else: ?>
                ...
                <div class="form-group">
                    <label for="reservation_name">Nom complet</label>
                    <input type="text" name="name" id="reservation_name" class="form-control" placeholder="Votre nom"
                        autocomplete="off" />
                </div>
                ...
                <div class="form-row">
                    <div class="form-group half">
                        ...
                        <input type="email" name="email" id="reservation_email" class="form-control"
                            placeholder="exemple@email.com" autocomplete="off" />
                    </div>
                    <div class="form-group half">
                        <label for="reservation_seats">Places</label>
                        <input type="number" name="seats" id="reservation_seats" class="form-control" value="1" min="1"
                            autocomplete="off" />
                    </div>
                </div>
            ...
            <?php if (!empty($reservations)): ?>
                <?php foreach ($reservations as $r): ?>
                    <div class="content-card">
                        ...
                        <div class="card-user">✉️ <?php echo htmlspecialchars($r['email']); ?> •
                            <?php echo htmlspecialchars($r['seats']); ?> place(s)</div>
                        <p class="card-desc">Créée le
                            <?php echo htmlspecialchars(date('d/m/Y H:i', strtotime($r['created_at']))); ?></p>
